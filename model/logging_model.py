...
class EmbeddingSimilarityEvaluator(SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        for i in range(len(self.sentences1)):
            score = cosine_similarity_manual(torch.Tensor(embeddings1[i]),torch.Tensor(embeddings2[i]))
            score =score.cpu().detach().numpy()
            test_scores.append(score)


        test_scores = np.array(test_scores) 
        y_pred = np.where(test_scores>=0.6, 1, 0)
        y_label = np.array(labels)
        y_label = np.where(y_label>=0.6, 1, 0)

        # try:      
        print(classification_report(y_label, y_pred))
        wandb.log({'f1': f1_score(y_label, y_pred)})
        # ################## 여기는 내가 추가####################
